    ...
    <div className={`bg-white rounded-lg border border-gray-200 p-4 mb-6 font-[Quicksand] ${className}`}>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          {onSearch && (
            <div className="relative flex-1 max-w-md">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2 border border-[#A73249] rounded-lg bg-[#E8E1D8] text-[#3D1609] placeholder-[#39312F] focus:outline-none focus:ring-2 focus:ring-[#A73249] focus:border-transparent transition-colors"
              />
            </div>
          )}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {/* Indicador de columnas visibles */}
          {visibleColumnsCount && totalColumnsCount && (
            <div className="hidden sm:flex items-center gap-1 px-2 py-1 bg-gray-100 rounded text-xs text-gray-600">
              <Columns className="w-3 h-3" />
              <span>{visibleColumnsCount}/{totalColumnsCount}</span>
            </div>
          )}
          ...
          {actions.canToggleColumns && onColumnToggle && (
            <ActionButton 
              variant="ghost" 
              icon={Columns} 
